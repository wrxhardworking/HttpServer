...
void Logger::log(LogLever::Lever lever_, LogEvent::ptr event){
    if(lever_>=m_lever_){//日志等级大于本身等级
        ...
        if((log=m_formatter_->format(shared_from_this(),lever_,event)).empty())
        {
            LOG_ERROR<<"log append failed";
        }
...
LogEvent::LogEvent(std::shared_ptr<Logger>logger,LogLever::Lever lever,std::string_view filename,uint32_t line,uint32_t elapse,uint32_t threadid,uint32_t fiber,int32_t time)
        :m_logger_(std::move(logger)),m_lever_(lever),m_filename_(filename.data()),m_line_(line),m_elapse_(elapse),m_threadid_(threadid),m_fiberid_(fiber),m_time_(time){
}
...
LogEvent& LogEvent::operator<<(const char* content) {
    m_content_.append(content);
    return *this;
}
LogEvent& LogEvent::operator<<(int content) {
